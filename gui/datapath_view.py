...
class DatapathView(tk.Frame):
    ...
    def _draw_layout(self):
        ...
        def make_reg(name, row):
            ...
            mid_y = (y1 + y2) / 2
            if name != "AC":
                # Input to register from left side of bus
                self.canvas.create_line(
                    self.bus_left_x, mid_y, self.reg_x1, mid_y,
                    width=2, fill="#888", arrow=tk.LAST
                )
            # Output from register to right side of bus
            self.canvas.create_line(
    ...
    def _draw_alu_block(self):
        ...
        # E flip-flop box (just decorative)
    ...
    def _draw_mem_addr_line(self):
        ...
        # Address line from AR down towards MEM
        self.canvas.create_line(
    ...
    def update_from_machine(self, machine, changed_set, micro_op_str=""):
        ...
        # ALU-related ops (AC and DR interaction)
        alu_ops = ["AC <- DR", "AC <- AC + DR", "AC <- AC & DR", "AC <- AC ∧ DR"]
        alu_event = any(p in m for p in alu_ops)

        # Register-reference operations (T3: CLA, CLE, etc.)
        reg_ref = m.startswith("T3:") and any(
            k in m for k in ["CLA", "CLE", "CMA", "CME", "CIR", "CIL", "INC", "SPA", "SNA", "SZA", "SZE", "HLT"]
        )

        # Memory reference: be a bit stricter than just "M[AR]"
        ...
        # Self-modifying / internal increments like R <- R + 1 or R <- 0
        ...
        if "<-" in m and not alu_event:
            first = m.split(",", 1)[0]  # focus on first micro-op if multiple
            if "<-" in first:
                # Split only on the FIRST "<-" to avoid "too many values to unpack"
        ...
        # Highlight registers that changed this micro-op
